...
public class GetRechnungFilterCommand implements ICommand {
	...
	public Response handleRequest(Uri uri, Request request) {
		...
		try {
			...
			RechnungFilter rf = (RechnungFilter) decoder.readObject();
			decoder.close();

			final XMLEncoder encoder = new XMLEncoder(new BufferedOutputStream(
					temp));
			encoder.writeObject(rd.getFilterRechnung(rf));
			encoder.close();
